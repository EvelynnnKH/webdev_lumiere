    ...
    <div class="container mx-auto max-w-4xl">
        ...
        <div class="form-container">
            ...
            <div class="form-content">
                <form action="{{ route('edit_product', ['product_id' => $product->product_id]) }}" method="POST" enctype="multipart/form-data" class="form-grid">
                    @csrf
                    @method('put') {{-- Penting untuk metode HTTP PUT/PATCH di Laravel --}}
                    ...
                    <div class="form-group">
                        ...
                        id="name" name="name" value="{{ old('name', $product->name) }}" placeholder="Enter product name">
                        @error('name')
                    ...
                    <div class="form-group">
                        ...
                        placeholder="Describe your product features, benefits, and specifications...">{{ old('description', $product->description) }}</textarea>
                        @error('description')
                            <div class="error-message">{{ $message }}</div> 
                        @enderror
                    </div>
                    ...
                    <div class="form-row">
                        <div class="form-group">
                            ...
                            <select name="category_id" id="category_id" class="form-input">
                                @foreach($category as $cat) {{-- Ganti $category menjadi $cat untuk menghindari konflik nama variabel --}}
                                    <option value="{{ $cat->category_id }}" {{ old('category_id', $product->category_id) == $cat->category_id ? 'selected' : '' }}>
                                        {{ $cat->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category_id') {{-- Tambahkan error handling untuk category_id --}}
                                <div class="error-message">{{ $message }}</div>
                            @enderror
                        </div>
